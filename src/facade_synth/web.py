...
<div><label>Exact output target</label><input id="count" type="number" min="1" value="8"></div><div><label>Seed</label><input id="seed" type="number" value="0"></div>
...
</div><p class="muted">View family: frontal, light/medium-oblique, strong-oblique. Occlusion: clear, 0–15%, 15–30%.</p>
...
async function preflight(){try{const v=await api('/api/preflight');document.getElementById('preflight').textContent=`ready: Blender ${v.blender_version}, NumPy ${v.numpy_version}`}catch(e){document.getElementById('preflight').textContent=`environment not ready: ${e.message}`}}
function payload(){return {task:val('task'),output_target:number('count'),seed:number('seed'),render_width:number('width'),render_height:number('height'),daylight_profile:val('daylight'),split_ratio:{train:.7,validation:.15,test:.15},building_use_distribution:{residential:number('residential'),office:number('office'),commercial:number('commercial'),mixed_use:number('mixed')},asset_paths:val('assets').split('\n').map(x=>x.trim()).filter(Boolean)}}
